...
let $contOperaciones =$("#contenedor-operaciones");
// box nueva operacion
...
const $btnEliminarOperacion = $(".eliminarOp");
// vista balance - totales
const $totalIngresos = $("#ganancias-total");
const $totalGastos = $("#gastos-total");
const $saldoTotal = $("#balance-total");
let $detalleOperaciones = $("#detalle-operaciones");
const $titulosDetalle = $("#titulos-detalle");
// filtros
const $ocultarFiltros = $(".filtros-ocultos");
const $contenedorFiltros = $("#contenedor-filtros");
const $ocultarFiltrosTxt = $("#ocultos-filtros-text");
...
let $boxContenedorCat = $(".box-contenedor-categorias");
// reportes
...
const estableOp = () => {
  for (const {descripcion, monto, tipo, categoria, fecha} of listaOperaciones) {
    $titulosDetalle.classList.remove("is-hidden");
    $contOperaciones.innerHTML += `
    <!-- Contenido tabla -->
    ...
    <div class="column is-size-7-mobile ${tipo === "ingresos" ? "has-text-success" : "has-text-danger"
    }">${tipo === "ingresos" ? "+" : "-"}$${monto}</div>
    <div class="column is-flex">
    <button class="button is-ghost is-small editarOp">Editar</button>
    <button class="button is-ghost is-small eliminarOp id="eliminaop"">Eliminar</button>
    </div)
    </div>
    `;
    const btnEliminar = $contOperaciones.querySelector(".eliminarOp");
      btnEliminar.onclick = function () {
        eliminarOperacion($contOperaciones);
      };
      const btnEditar = $contOperaciones.querySelector(".editarOp");
      btnEditar.onclick = function () {
        editarItem($contOperaciones);
      };
      $detalleOperaciones.append($contOperaciones);
  }
};
...
const agregarOperaciones = () => {
  ...
  <div class="column is-hidden-mobile">${operacion.fecha}</div>
  <div class="column is-size-7-mobile ${operacion.tipo === "ingresos" ? "has-text-success" : "has-text-danger"
  }">${operacion.tipo === "ingresos" ? "+" : "-"}$${operacion.monto}</div>
  <div class="column is-flex">
  <button class="button is-ghost is-small editarOp">Editar</button>
  <button class="button is-ghost is-small eliminarOp id="eliminaop"">Eliminar</button>
  </div)
...
//$btnEliminarOperacion.addEventListener("click", eliminarOperacion);
// agregar nueva operacion
...
$filtroOrdenX.addEventListener("change", filtros);
// agregar categoria
